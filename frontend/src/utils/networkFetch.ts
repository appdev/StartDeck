...
export const installNetworkFetchPatch = () => {
  ...
  const patchedFetch: typeof window.fetch & { [patchedFlag]?: boolean } = ((
    ...
    return nativeFetch(input, { ...init, signal: controller.signal }).catch(
      async (error) => {
  ...
  (
    patchedFetch as typeof window.fetch & {
      [patchedFlag]: boolean;
      addNetworkErrorListener?: typeof networkErrorListeners.add;
      removeNetworkErrorListener?: typeof networkErrorListeners.delete;
    }
  ).addNetworkErrorListener = (fn) => networkErrorListeners.add(fn);
  (
    patchedFetch as typeof window.fetch & {
      [patchedFlag]: boolean;
      removeNetworkErrorListener?: typeof networkErrorListeners.delete;
    }
  ).removeNetworkErrorListener = (fn) => networkErrorListeners.delete(fn);
