...
describe("useWidgetResize scoped finite size rules", () => {
  it("snaps sizes to whole fixed grid units", () => {
    expect(snapWidgetSizeValue(1.24)).toBe(1);
    expect(snapWidgetSizeValue(1.51)).toBe(2);
  });

  it("keeps scoped size options visible but disables widths over a one-column runtime", () => {
  ...
  it("applies the scoped family max before commit", () => {
    const state = resolveWidgetSizeState({
  ...
  it("marks non-family sizes as unsupported and resolves to the nearest scoped size", () => {
    const state = resolveWidgetSizeState({
      widgetType: "calculator",
      deviceKey: "desktop",
      runtimeCols: 4,
  ...
  it("uses the scoped five scoped size options and preserves captured orientation", () => {
    const state = resolveWidgetSizeState({
