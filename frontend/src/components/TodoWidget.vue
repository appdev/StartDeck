...
const pollRemote = async (force = false) => {
  ...
  try {
    const res = await fetch(
      `/api/widgets/${encodeURIComponent(props.widget.id)}`,
      {
...
onMounted(() => {
  ...
  if (
    store.isLogged &&
    (!props.widget.data || props.widget.data.length === 0) &&
    localBackup.value.length > 0
  ) {
...
<template>
  ...
  >
    <div
      class="font-bold text-white text-xs mb-2 flex justify-between items-center"
    >
    ...
    >
      <div class="text-center px-6">
        <div class="text-sm font-bold text-white">登录后使用待办</div>
      </div>
    </div>

    <template v-else>
      <div
        class="flex-1 overflow-y-auto space-y-1 scrollbar-hide"
      ...
      >
        ...
        <div
          v-if="!widget.data?.length"
          class="text-xs text-white/50 text-center py-2"
        >
          无待办事项
        </div>
      </div>
