...
describe("GridPanel home search styles", () => {
  it("keeps only the selected search engine masked with the search box color", () => {
    const searchBlock = getRuleBlock(gridPanelSource, ".sd-home-search");
    ...
    expect(wrapBlock).toContain("--sd-home-search-engine-selected-bg");
    expect(wrapBlock).toContain("var(--sd-theme-grid-panel-surface-02) 42%");
    expect(wrapBlock).toContain("--sd-home-search-engine-text");
    expect(wrapBlock).toContain("--sd-home-search-engine-selected-text");
