...
const processIconError = () => {
  ...
  if (
    ...
    console.log("Icon load failed, trying StartPage iconserver fallback:", val);
    try {
...
const cacheIconToLocal = async (icon: string): Promise<{ path: string | null; error: string | null }> => {
  ...
  if (trimmed.startsWith("data:")) {
    ...
    if (normalized) {
      if (trimmed.startsWith("icons/") || trimmed.startsWith("/icons/") || trimmed.startsWith("/api/site/icon")) {
