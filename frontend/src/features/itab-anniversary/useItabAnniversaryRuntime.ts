...
export const anniversaryBackgroundImages = Array.from(
  { length: ITAB_ANNIVERSARY_IMAGE_COUNT },
  (_, index) => {
...
export const anniversaryEditorSizes: Array<
  Extract<ItabWidgetSizeKey, "2x2" | "2x4">
> = ["2x2", "2x4"];
export const anniversaryPreviewSizes: ItabWidgetSizeKey[] = [
  "1x1",
  "1x2",
  "2x1",
  "2x2",
...
export const anniversaryTemplates: ItabAnniversaryTemplate[] = [
  {
    ...normalizeItabAnniversaryWidgetData({}),
    id: "life",
  },
  {
    ...normalizeItabAnniversaryWidgetData({
      ...
      backgroundImage: ITAB_ANNIVERSARY_DEFAULT_BACKGROUND_IMAGE,
    }),
    id: "payday",
  },
  {
...
export const anniversaryDays = (
  ...
  repeat: ItabAnniversaryWidgetData["repeat"] = "不重复",
  now = new Date(),
) => {
...
export const anniversaryTemplateStyle = (
  ...
  const imageLayer =
    template.backgroundMode === "image"
      ? `url("${template.backgroundImage || ITAB_ANNIVERSARY_DEFAULT_BACKGROUND_IMAGE}")`
      : "none";
  const maskLayer =
...
export const anniversaryUsesCalendar = (
  template: Pick<ItabAnniversaryWidgetData, "sizeKey" | "eventName"> & {
    id?: string;
  },
) =>
...
export const useItabAnniversaryRuntime = (
  widget: ComputedRef<WidgetConfig>,
  emitUpdate?: (data: ItabAnniversaryWidgetData) => void,
) => {
  const data = computed(() =>
    normalizeItabAnniversaryWidgetData(widget.value.data),
  );
  const cardTemplate = computed<ItabAnniversaryTemplate>(() => ({
    ...data.value,
    id: "current",
  }));
  ...
  const updateData = (patch: Partial<ItabAnniversaryWidgetData>) => {
    emitUpdate?.(
      normalizeItabAnniversaryWidgetData({
        ...data.value,
        ...patch,
