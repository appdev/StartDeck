...
describe("weather opened panel", () => {
  it("draws its own source weather surface instead of relying on shell color", () => {
    ...
    expect(source).toContain('url("/sd/weather/background/rain_d.webp")');
    expect(source).toContain(
  ...
  it("uses source translucent surfaces inside the opened panel", () => {
    ...
    expect(openedSource).toContain(
      "--sd-theme-weather-weather-opened-panel-surface-07",
    );
    expect(themeSource).toContain(
      "--sd-theme-weather-weather-opened-panel-surface-07: #ffffff08;",
    );
    expect(openedSource).toContain(".weather-hour-track span:hover");
    expect(openedSource).toContain(
      "--sd-theme-weather-weather-opened-panel-surface-08",
    );
    expect(themeSource).toContain(
      "--sd-theme-weather-weather-opened-panel-surface-08: #ffffff14;",
    );
    expect(openedSource).toContain(".weather-day-grid article:hover");
    expect(openedSource).toContain(
      "--sd-theme-weather-weather-opened-panel-surface-09",
    );
    expect(themeSource).toContain(
      "--sd-theme-weather-weather-opened-panel-surface-09: #ffffff0f;",
    );
  });
  ...
  it("binds the weather skin to the current city card", () => {
    ...
    expect(themeSource).toContain(
      "--sd-theme-weather-weather-opened-panel-surface-10: #154280;",
    );
    expect(source).toContain("border-radius: 12px;");
    expect(source).toContain(
      "--sd-theme-weather-weather-opened-panel-text-14",
    );
    expect(themeSource).toContain(
      "--sd-theme-weather-weather-opened-panel-text-14: #d5d5d5;",
    );
  });
});
