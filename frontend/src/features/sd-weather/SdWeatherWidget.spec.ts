...
import SdWeatherOpenedPanel from "./SdWeatherOpenedPanel.vue";
import SdWeatherWidget from "./SdWeatherWidget.vue";
import {
  resetSdWeatherRuntimeForTests,
  resolveSdWeatherSkinClass,
} from "./useSdWeatherRuntime";
import { resetSdIpRuntimeForTests } from "@/features/sd-ip/useSdIpRuntime";
...
describe("weather widget", () => {
  ...
  it("loads current weather through the backend proxy and renders scoped size UI", async () => {
    ...
    const wrapper = mount(SdWeatherWidget, {
      props: {
        sizeKey: "1x2",
        widget: {
          id: "weather",
    ...
    expect(wrapper.attributes("data-sd-weather-size")).toBe("1x2");
    expect(
      wrapper
  ...
  it("does not render the old default city when no weather data is available", async () => {
    ...
    expect(wrapper.attributes("data-sd-weather-source-status")).toBe("error");
  ...
  it("maps source weather codes to fixed source skin classes", () => {
    ...
    expect(resolveSdWeatherSkinClass({ ...daySample, code: "150" })).toBe(
      "weather-sunny_d",
    );
    expect(resolveSdWeatherSkinClass({ ...daySample, code: "304" })).toBe(
      "weather-rain_d",
    );
    expect(resolveSdWeatherSkinClass({ ...daySample, code: "999" })).toBe(
      "weather-other",
    );
    expect(resolveSdWeatherSkinClass({ ...daySample, code: "" })).toBe(
      "weather-sunny_d",
    );
    expect(
      resolveSdWeatherSkinClass({
  ...
  it("keeps the closed weather cover on source fixed skin colors", () => {
    const source = readFileSync(
      "src/features/sd-weather/SdWeatherWidget.vue",
      "utf8",
    );
    ...
    expect(themeSource).toContain(
      "--sd-theme-weather-weather-opened-panel-scene-base: #184482;",
    );
    expect(source).toContain(
      "color: var(--sd-theme-weather-weather-widget-text-01);",
    );
    expect(source).toContain(
  ...
  it("keeps current weather in runtime cache until the user opens it after five minutes", async () => {
    ...
    const openedBeforeTtl = mount(SdWeatherOpenedPanel, {
      props: { widget },
    });
    await flushPromises();
    expect(fetchMock).toHaveBeenCalledTimes(1);
    ...
    const openedAfterTtl = mount(SdWeatherOpenedPanel, {
      props: { widget },
    });
    await flushPromises();
