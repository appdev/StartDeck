...
import ItabPomodoroWidget from "./ItabPomodoroWidget.vue";
...
describe("ItabPomodoroWidget", () => {
  ...
  it.each([
    ...
    const wrapper = mount(ItabPomodoroWidget, {
      props: {
    ...
    expect(wrapper.attributes("data-itab-pomodoro-size")).toBe(sizeKey);
    expect(wrapper.text()).toContain(text);
    expect(wrapper.find("img").attributes("src")).toContain(
      "/itab/widget/tomato/hailang.jpg",
    );
    wrapper.unmount();
  });
