...
import { normalizeSdPomodoroWidgetData } from "./sdPomodoroModel";
import {
  SD_POMODORO_DEFAULT_DURATION_SECONDS,
  SD_POMODORO_RUNTIME,
  type SdPomodoroPhase,
  type SdPomodoroTheme,
  type SdPomodoroWidgetData,
} from "./sdPomodoroTypes";

const POMODORO_AUDIO_BASE_PATH = "/sd-live-assets/tomato-audio";
const POMODORO_STORAGE_PREFIX = "startdeck:sd-pomodoro";

export const pomodoroThemes: SdPomodoroTheme[] = [
  {
    name: "海浪",
    key: "hailang",
    path: "hailang.jpg",
...
export const pomodoroImageUrl = (
  ...
  return `/sd/widget/tomato/${path}?x-oss-process=image/resize,limit_0,m_fill,${dimensions}/format,jpg`;
};

type RuntimeUpdateListener = (data: SdPomodoroWidgetData) => void;
...
const createState = (): PomodoroRuntimeState => ({
  hydrated: false,
  durationSeconds: ref(SD_POMODORO_DEFAULT_DURATION_SECONDS),
...
const readLocalData = (widgetId: string) => {
  ...
  try {
    ...
    return normalizeSdPomodoroWidgetData(JSON.parse(raw));
  } catch {
    return undefined;
  }
};

const writeLocalData = (widgetId: string, data: SdPomodoroWidgetData) => {
...
const chooseNewestData = (
  widgetData: SdPomodoroWidgetData,
  localData?: SdPomodoroWidgetData,
) => {
...
export const useSdPomodoroRuntime = (
  widget: ComputedRef<WidgetConfig>,
  emitUpdate: (data: SdPomodoroWidgetData) => void,
) => {
  const state = computed(() => getState(widget.value.id));
  const normalizedData = computed(() =>
    normalizeSdPomodoroWidgetData(widget.value.data),
  );
  ...
  const toWidgetData = (): SdPomodoroWidgetData => ({
  ...
  const applyDataToState = (data: SdPomodoroWidgetData) => {
    const current = state.value;
    const now = Date.now();
