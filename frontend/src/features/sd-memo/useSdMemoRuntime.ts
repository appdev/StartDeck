...
export interface SdMemoRuntimeOptions {
  remoteSync?: boolean;
...
export const formatSdMemoTime = (value: string) => {
...
export const useSdMemoRuntime = (
  widget: ComputedRef<WidgetConfig>,
  emitUpdate: (data: SdMemoWidgetData) => void,
  ...
  const canReadMemo = computed(() => !authRequired || !!widget.value);
  const canWriteMemo = computed(
    () => !authRequired || canWriteResource(store.isLogged),
  );
  const normalizedData = computed(() =>
    normalizeSdMemoWidgetData(widget.value.data),
  );
  ...
  const memoBackupKey = () => {
    const scope = store.isLogged
      ...
      : "guest";
    return `startdeck-sd-memo-backup-${scope}-${widget.value.id}`;
  };
  ...
  const readLocalBackup = () => {
    ...
    try {
      ...
      if (!raw) return [];
      return normalizeSdMemoWidgetData({ notes: JSON.parse(raw) }).notes;
    } catch {
      return [];
    }
  };

  const writeLocalBackup = (items: SdMemoNote[]) => {
    if (!store.isLogged || typeof localStorage === "undefined") return;
    try {
  ...
  const pushSocketUpdate = useDebounceFn((data: SdMemoWidgetData) => {
  ...
  const emitData = (data: SdMemoWidgetData, localMutation = false) => {
    if (localMutation) {
  ...
  const commitNotes = (
    ...
    const nextData: SdMemoWidgetData = {
  ...
  const updateNote = (
    noteId: string,
    patch: Partial<Pick<SdMemoNote, "title" | "body">>,
  ) => {
    commitNotes(
  ...
  const pollRemote = async (force = false) => {
    ...
    try {
      const raw = await fetchSdMemoWidgetData(widget.value.id, {
