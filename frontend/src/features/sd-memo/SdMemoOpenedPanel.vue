...
import { formatSdMemoTime, useSdMemoRuntime } from "./useSdMemoRuntime";
import type { SdMemoNote, SdMemoWidgetData } from "./sdMemoTypes";
...
const noteTitle = (note: SdMemoNote) => note.title || "未命名备忘录";
const noteListTime = (note: SdMemoNote) =>
  note.listTime ||
  formatSdMemoTime(note.updatedAt).slice(0, 16).replaceAll("-", "/");
...
<template>
  <div class="sd-memo-opened-panel" data-sd-memo-opened-panel>
    ...
    <section class="memo-content">
      <div class="memo-content-inner">
        <template v-if="activeNote">
          ...
          <p class="memo-meta">
            最后编辑：{{ formatSdMemoTime(activeNote.updatedAt) }}，创建：{{
              formatSdMemoTime(activeNote.createdAt)
            }}
          </p>
        </template>
...
<style scoped>
.sd-memo-opened-panel {
  display: flex;
  width: 998px;
  height: 600px;
  overflow: hidden;
  background: var(--sd-theme-memo-memo-opened-panel-surface-01);
  color: var(--sd-theme-memo-memo-opened-panel-text-01);
  font-size: 14px;
  line-height: 21px;
}
...
.memo-tabs h2 {
  ...
  margin: 0 0 12px;
  padding-left: 4px;
  color: var(--sd-theme-memo-memo-opened-panel-text-02);
  font-size: 18px;
  font-weight: 700;
  line-height: 27px;
}
...
.memo-sort-caret.desc {
  border-top: 6px solid
    var(--sd-theme-memo-memo-opened-panel-accent-border-01);
}

.memo-sort-caret.asc {
  border-bottom: 6px solid var(--sd-theme-memo-memo-opened-panel-border-01);
}
...
.memo-tabs-item:hover {
  background: var(--sd-theme-memo-memo-opened-panel-surface-02);
}

.memo-tabs-item.active {
  background: var(--sd-theme-memo-memo-opened-panel-surface-03);
  color: var(--sd-theme-memo-memo-opened-panel-accent-text-01);
}
...
.memo-tabs-item time {
  display: block;
  overflow: hidden;
  color: var(--sd-theme-memo-memo-opened-panel-text-03);
  font-size: 12px;
  line-height: 18px;
  text-overflow: ellipsis;
  white-space: nowrap;
...
.memo-note-action:focus-visible {
  outline: 2px solid
    var(--sd-theme-memo-memo-opened-panel-accent-border-02);
  outline-offset: 1px;
}
...
.memo-add {
  ...
  border: 0;
  border-radius: 999px;
  background: var(--sd-theme-memo-memo-opened-panel-accent-surface-01);
  color: var(--sd-theme-memo-memo-opened-panel-text-04);
  box-shadow: var(--sd-theme-memo-memo-opened-panel-shadow-01) 0 0 5px 2px;
  cursor: pointer;
}
...
.memo-textarea-wrap {
  ...
  width: 778px;
  height: 528px;
  border-top: 1px solid var(--sd-theme-memo-memo-opened-panel-border-02);
}
