...
import { MEMO_SVG_PATHS, useSdMemoRuntime } from "./useSdMemoRuntime";
...
const emit = defineEmits<{
  updateData: [widget: WidgetConfig, data: SdMemoWidgetData];
}>();
...
const fixedMemoBody = (note: SdMemoNote) => note.body.trim() || "无内容";
</script>

<template>
  <div
    v-if="widget && runtime.fixedNotes.value.length"
    class="sd-memo-fixed-layer"
    data-sd-memo-fixed-layer
...
.sd-memo-fixed-layer {
  ...
  padding: 0;
  overflow-x: auto;
  color: var(--sd-theme-memo-memo-fixed-layer-text-01);
  font-size: 13px;
  line-height: 19.5px;
  list-style: none;
}
...
.notes-fixed-item {
  ...
  box-shadow: 0 14px 32px var(--sd-theme-memo-memo-fixed-layer-shadow-01);
...
.notes-fixed-content time {
  display: inline;
  color: var(--sd-theme-memo-memo-fixed-layer-text-02);
  font-size: 12px;
  line-height: 16px;
}
