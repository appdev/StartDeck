...
import {
  createErrorSdIpResult,
  createLoadingSdIpResult,
...
import type { SdIpLookupResult, SdIpLookupStatus } from "./sdIpTypes";

const result = ref<SdIpLookupResult>(createLoadingSdIpResult());
const status = ref<SdIpLookupStatus>("idle");
const loading = ref(false);
const error = ref("");
const latencyMs = ref<number | null>(null);
const latencyStatus = ref<SdIpLookupStatus>("idle");
const latencyLoading = ref(false);
const latencyError = ref("");
const latencyIpKey = ref("");
const SD_IP_LATENCY_AUTO_REFRESH_MS = 5 * 60 * 1000;
let abortController: AbortController | null = null;
let latencyAbortController: AbortController | null = null;
let lookupInFlight: Promise<SdIpLookupResult> | null = null;
let requestSerial = 0;
let latencyRequestSerial = 0;
let latencyAutoRefreshTimer: number | null = null;
let latencyAutoRefreshSubscribers = 0;

const ipLatencyKey = (value: SdIpLookupResult) =>
  (value.queryIp || value.ip || value.clientIp || "").trim();

export const resetSdIpRuntimeForTests = () => {
  ...
  latencyError.value = "";
  latencyIpKey.value = "";
  result.value = createLoadingSdIpResult();
};

const applyLookupResult = (next: SdIpLookupResult) => {
...
const requestLookup = async () => {
  ...
  const promise = fetchSdIpLookup(false, controller.signal)
    .then((next) => {
    ...
    .catch((loadError) => {
      if (!controller.signal.aborted && serial === requestSerial) {
        result.value = createErrorSdIpResult(result.value);
        status.value = "error";
        error.value =
...
export const prefetchSdIpLocation = async () => {
  try {
...
export const useSdIpRuntime = () => {
  const load = async () => {
    return prefetchSdIpLocation();
  };
  ...
  const refreshLatency = async (force = true) => {
    ...
    try {
      const next = await fetchSdIpLatency(controller.signal);
  ...
  const address = computed(() => formatSdIpAddress(result.value));
  const area = computed(() => formatSdIpArea(result.value));
  const outerLocation = computed(() => formatSdIpOuterLocation(result.value));
  const network = computed(() => formatSdIpNetwork(result.value));
  const coordinate = computed(() => formatSdIpCoordinate(result.value));
  const mapEmbedUrl = computed(() => createSdIpMapEmbedUrl(result.value));
  const latencyLabel = computed(() =>
    formatSdIpLatency(latencyMs.value, latencyStatus.value),
  );
  const latencyValue = computed(() =>
    latencyMs.value === null ? "" : String(Math.round(latencyMs.value)),
  );
  const addressClass = computed(() => ({
    "is-long-address": isLongSdIpAddress(result.value),
  }));
