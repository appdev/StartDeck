...
import {
  ...
  buildSdCalendarSnapshot,
  solarDayFromKey,
} from "./useSdCalendarRuntime";

describe("calendar runtime", () => {
  it("derives the calendar snapshot from tyme4ts", () => {
    const snapshot = buildSdCalendarSnapshot(new Date(2026, 4, 20, 12));
  ...
  it("builds a fixed 42-day month grid with holiday and solar-term labels", () => {
    const grid = buildSdCalendarMonthGrid(2026, 5, new Date(2026, 4, 20, 12));
  ...
  it("can switch the month grid to Sunday-first ordering", () => {
    const grid = buildSdCalendarMonthGrid(
      2026,
      5,
  ...
  it("builds detail rows for arbitrary selected dates", () => {
    const detail = buildSdCalendarDetail(solarDayFromKey("2026-05-21"));
