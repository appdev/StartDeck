...
import { normalizeSdCalendarWidgetData } from "./sdCalendarModel";
import type {
  SdCalendarDay,
  SdCalendarDetail,
  SdCalendarSnapshot,
...
export type SdCalendarWeekStart = "monday" | "sunday";
...
export const buildSdCalendarDay = (
  solarDay: SolarDay,
  todayKey: string,
  currentMonth: number,
): SdCalendarDay => {
...
export const buildSdCalendarMonthGrid = (
  year: number,
  month: number,
  today = new Date(),
  weekStart: SdCalendarWeekStart = "monday",
) => {
  ...
  return Array.from({ length: MONTH_CELL_COUNT }, (_, index) =>
    buildSdCalendarDay(startDay.next(index), todayKey, month),
  );
};

export const buildSdCalendarDetail = (
  solarDay: SolarDay,
): SdCalendarDetail => {
  const lunarDay = solarDay.getLunarDay();
...
export const buildSdCalendarSnapshot = (
  today = new Date(),
): SdCalendarSnapshot => {
  const solarToday = solarDayFromDate(today);
  const todayKey = dateKeyFromDate(today);
  return {
    today: buildSdCalendarDay(solarToday, todayKey, solarToday.getMonth()),
    todayDetail: buildSdCalendarDetail(solarToday),
    monthTitle: `${solarToday.getYear()}年${solarToday.getMonth()}月`,
    grid: buildSdCalendarMonthGrid(
      solarToday.getYear(),
      solarToday.getMonth(),
      today,
    ),
...
export const useSdCalendarRuntime = (
  widgetRef: ComputedRef<WidgetConfig>,
  onUpdateData?: (data: SdCalendarWidgetData) => void,
) => {
  const now = ref(new Date());
  const data = computed(() =>
    normalizeSdCalendarWidgetData(widgetRef.value.data),
  );
  const snapshot = computed(() => buildSdCalendarSnapshot(now.value));
