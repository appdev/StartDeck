...
import SdFoodPickerWidget from "./SdFoodPickerWidget.vue";
...
describe("food picker widget", () => {
  ...
  it.each(["1x1", "1x2", "2x1", "2x2", "2x4"] as const)(
    ...
    (sizeKey) => {
      const wrapper = mount(SdFoodPickerWidget, {
        props: {
          widget: createDefaultSdFoodPickerWidget(),
          sizeKey,
        },
      });

      expect(wrapper.attributes("data-sd-food-picker-size")).toBe(sizeKey);
  ...
  it("keeps text colors on food picker theme tokens", () => {
    const source = readFileSync(
      "src/features/sd-food-picker/SdFoodPickerWidget.vue",
      "utf8",
    );
