...
import ItabCalendarOpenedPanel from "./ItabCalendarOpenedPanel.vue";

const mountPanel = () =>
  mount(ItabCalendarOpenedPanel, {
    props: {
      widget: createDefaultItabCalendarWidget(),
    },
  });

describe("ItabCalendarOpenedPanel", () => {
  beforeEach(() => {
    vi.useFakeTimers();
  ...
  it("renders the selected day detail and fixed month grid", () => {
    ...
    expect(wrapper.attributes("data-itab-calendar-selected-date")).toBe(
      "2026-05-20",
    );
  ...
  it("switches months and selects a solar term day", async () => {
    ...
    expect(wrapper.attributes("data-itab-calendar-view-month")).toBe("2026-06");
    expect(wrapper.attributes("data-itab-calendar-selected-date")).toBe(
      "2026-06-01",
    );
  ...
  it("keeps the current month visible when selecting an adjacent-month cell", async () => {
    ...
    expect(wrapper.attributes("data-itab-calendar-view-month")).toBe("2026-05");
    expect(wrapper.attributes("data-itab-calendar-selected-date")).toBe(
      "2026-06-01",
    );
    expect(wrapper.text()).toContain("2026-06-01 周一");
  ...
  it("opens year and month pickers from the toolbar", async () => {
    ...
    expect(wrapper.attributes("data-itab-calendar-view-month")).toBe("2027-05");
    expect(wrapper.attributes("data-itab-calendar-selected-date")).toBe(
      "2027-05-01",
    );
    expect(wrapper.find("[role='listbox'][aria-label='年份']").exists()).toBe(
    ...
    expect(wrapper.attributes("data-itab-calendar-view-month")).toBe("2027-12");
    expect(wrapper.attributes("data-itab-calendar-selected-date")).toBe(
      "2027-12-01",
    );
  });
  ...
  it("opens tools, calculates dates, workdays, and festival details", async () => {
    ...
    expect(wrapper.find("[data-itab-calendar-tools-panel]").exists()).toBe(
      true,
    );
