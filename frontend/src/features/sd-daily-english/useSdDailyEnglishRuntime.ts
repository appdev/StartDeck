...
export const SD_DAILY_ENGLISH_API_REFERENCE =
  "https://api.timelessq.com/english-sentence";
export const SD_DAILY_ENGLISH_PROVIDER_REFERENCE =
  "https://api.timelessq.com";

export const createEmptySdDailyEnglishEntry = (
  sourceStatus: SdDailyEnglishEntry["sourceStatus"] = "loading",
): SdDailyEnglishEntry => ({
  mode: "",
  sentence: "",
  translation: "",
...
const entry = ref<SdDailyEnglishEntry>(createEmptySdDailyEnglishEntry());
const loading = ref(false);
const error = ref("");
let abortController: AbortController | null = null;
let requestSerial = 0;

export const useSdDailyEnglishRuntime = () => {
  const load = async (refresh = false) => {
    ...
    try {
      const next = await fetchSdDailyEnglish(refresh, controller.signal);
      ...
      if (!controller.signal.aborted && serial === requestSerial) {
        entry.value = createEmptySdDailyEnglishEntry("error");
        error.value =
