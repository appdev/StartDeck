...
import { CODELIFE_TODAY_MOVIE_URL } from "@/utils/upstreamAllowlist";
import { fetchSdMovieCalendar } from "./sdMovieCalendarApi";
import type { SdMovieCalendarEntry } from "./sdMovieCalendarTypes";

export const SD_MOVIE_CALENDAR_API_REFERENCE = CODELIFE_TODAY_MOVIE_URL;
...
const createLoadingEntry = (): SdMovieCalendarEntry => ({
  date: "",
  day: "",
  monthLabel: "",
  weekday: "",
...
const createErrorEntry = (): SdMovieCalendarEntry => ({
  ...createLoadingEntry(),
  sourceStatus: "error",
});

const normalizeEntry = (
  entry: SdMovieCalendarEntry,
): SdMovieCalendarEntry => ({
  ...entry,
...
const entry = ref<SdMovieCalendarEntry>(createLoadingEntry());
const loading = ref(false);
const error = ref("");
let abortController: AbortController | null = null;
let requestSerial = 0;

export const resetSdMovieCalendarRuntimeForTests = () => {
...
export const useSdMovieCalendarRuntime = () => {
  const load = async (refresh = false) => {
    ...
    try {
      const next = normalizeEntry(
        await fetchSdMovieCalendar(refresh, controller.signal),
      );
