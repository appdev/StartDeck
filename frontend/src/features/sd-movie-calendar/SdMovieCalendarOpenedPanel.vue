...
import {
  SD_MOVIE_CALENDAR_API_REFERENCE,
  useSdMovieCalendarRuntime,
} from "./useSdMovieCalendarRuntime";

defineProps<{
  widget: WidgetConfig;
}>();

const runtime = useSdMovieCalendarRuntime();
...
<template>
  <section
    class="sd-movie-calendar-opened-panel"
    :style="runtime.entryStyle.value"
    :data-sd-movie-calendar-api="SD_MOVIE_CALENDAR_API_REFERENCE"
    :data-sd-movie-calendar-date="runtime.entry.value.date"
    :data-sd-movie-calendar-title="runtime.entry.value.movieTitle"
    :data-sd-movie-calendar-source-status="runtime.sourceStatus.value"
  >
    ...
    <a
      ...
      data-sd-hotspot-id="movie-source"
      data-sd-action="open-source"
...
<style scoped>
.sd-movie-calendar-opened-panel {
  position: relative;
  display: grid;
  ...
  color: var(
    --sd-theme-movie-calendar-movie-calendar-opened-panel-accent-text-01
  );
  font-family:
    "PingFang SC",
    -apple-system,
...
.movie-calendar-panel-bg {
  ...
  background:
    linear-gradient(
      var(
        --sd-theme-movie-calendar-movie-calendar-opened-panel-surface-01
      ),
      var(
        --sd-theme-movie-calendar-movie-calendar-opened-panel-accent-surface-01
      )
    ),
    var(--movie-poster-image, none) center/cover no-repeat,
    var(--sd-theme-movie-calendar-movie-calendar-opened-panel-surface-02);
  filter: blur(15px);
}
...
.movie-calendar-panel-bg::after {
  ...
  inset: 0;
  background: var(
    --sd-theme-movie-calendar-movie-calendar-opened-panel-surface-03
  );
  content: "";
}
...
.movie-calendar-panel-poster {
  ...
  background:
    linear-gradient(
      160deg,
      var(
        --sd-theme-movie-calendar-movie-calendar-opened-panel-surface-04
      ),
      var(--sd-theme-movie-calendar-movie-calendar-opened-panel-surface-05)
    ),
    var(
      --movie-bg-color,
      var(--sd-theme-movie-calendar-movie-calendar-opened-panel-surface-06)
    );
  color: var(
    --sd-theme-movie-calendar-movie-calendar-opened-panel-text-01
  );
  font-size: 38px;
  font-weight: 700;
  text-align: center;
...
.movie-calendar-panel-rating-star {
  ...
  width: 55px;
  height: 11px;
  background-image: url("../../assets/sd/movie-calendar/rating_s@2x.png");
...
.movie-calendar-panel-quote {
  ...
  color: var(
    --sd-theme-movie-calendar-movie-calendar-opened-panel-text-02
  ) !important;
...
.movie-calendar-panel-intro {
  ...
  color: var(
    --sd-theme-movie-calendar-movie-calendar-opened-panel-text-03
  ) !important;
...
.movie-calendar-panel-source {
  ...
  background: transparent;
  color: var(
    --sd-theme-movie-calendar-movie-calendar-opened-panel-accent-text-02
  );
  font-size: 12px;
  font-weight: 400;
  line-height: 19.2px;
