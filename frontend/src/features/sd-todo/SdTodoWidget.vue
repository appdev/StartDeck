...
import type { SdTodoSizeKey, SdTodoWidgetData } from "./sdTodoTypes";

const TODO_ICON_URL = "/sd/todo/todo.svg";
...
const runtime = useSdTodoRuntime(
  widgetRef,
...
<template>
  <span
    class="sd-todo-widget"
    data-sd-todo-widget
    :data-sd-todo-size="sizeKey"
  >
...
.todo-icon-asset {
  position: relative;
  background: var(--sd-theme-todo-todo-widget-surface-01);
}
...
.todo-icon-asset img {
  ...
  width: 100%;
  height: 100%;
  background-color: var(--sd-theme-todo-todo-widget-accent-surface-01);
  object-fit: contain;
}
...
.todo-icon-content {
  ...
  overflow: hidden;
  background: var(--sd-theme-todo-todo-widget-surface-01);
  color: var(--sd-theme-todo-todo-widget-text-01);
}
...
.todo-icon-title {
  ...
  gap: 8px;
  padding: 0 8.4px;
  color: var(--sd-theme-todo-todo-widget-accent-text-01);
  font-size: 12px;
  font-weight: 700;
  line-height: 18px;
}
...
.todo-icon-row i {
  ...
  margin-right: 7px;
  border-radius: 3px;
  background: var(--sd-theme-todo-todo-widget-surface-02);
}
...
.todo-icon-check {
  ...
  margin-right: 10px;
  padding: 0;
  border: 2px solid var(--sd-theme-todo-todo-widget-border-01);
  border-radius: 4px;
  background: var(--sd-theme-todo-todo-widget-surface-01);
  cursor: pointer;
}
...
.todo-icon-check:hover {
  border-color: var(--sd-theme-todo-todo-widget-accent-border-01);
}
...
.todo-icon-check.is-done {
  ...
  place-items: center;
  border-color: var(--sd-theme-todo-todo-widget-accent-border-01);
  background: var(--sd-theme-todo-todo-widget-accent-surface-01);
  color: var(--sd-theme-todo-todo-widget-surface-01);
}
...
.todo-icon-content.is-board .todo-icon-delete:focus-visible {
  background: color-mix(
    in srgb,
    var(--sd-theme-todo-todo-widget-accent-surface-01) 16%,
    transparent
  );
  color: var(--sd-theme-todo-todo-widget-accent-text-01);
  opacity: 1;
...
.todo-icon-content.is-board .todo-icon-row.is-done {
  color: color-mix(
    in srgb,
    var(--sd-theme-todo-todo-widget-text-01) 62%,
    transparent
  );
}
