...
import SdTodoWidget from "./SdTodoWidget.vue";
...
describe("todo widget", () => {
  ...
  it("renders icon-only sizes with the source Todo icon", () => {
    const wrapper = mount(SdTodoWidget, {
      props: {
        widget,
        sizeKey: "1x1",
      },
  ...
  it("renders a 4x4 task board and toggles tasks without dropping size", async () => {
    ...
    expect(wrapper.attributes("data-sd-todo-size")).toBe("4x4");
  ...
  it("keeps 4x4 delete buttons right aligned and hover gated", () => {
    const source = readFileSync(
      "src/features/sd-todo/SdTodoWidget.vue",
      "utf8",
    );
