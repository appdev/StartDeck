...
import { fetchSdTodoWidgetData } from "./sdTodoApi";
import { normalizeSdTodoWidgetData } from "./sdTodoModel";
import type { SdTodoTask, SdTodoWidgetData } from "./sdTodoTypes";

const TODO_POLL_INTERVAL_MS = 10000;
const TODO_POLL_TIMEOUT_MS = 8000;
const TODO_LOCAL_CHANGE_GRACE_MS = 8000;

export interface SdTodoRuntimeOptions {
  remoteSync?: boolean;
}

export const useSdTodoRuntime = (
  widget: ComputedRef<WidgetConfig>,
  emitUpdate: (data: SdTodoWidgetData) => void,
  options: SdTodoRuntimeOptions = {},
) => {
  ...
  const normalizedData = computed(() =>
    normalizeSdTodoWidgetData(widget.value.data),
  );
  ...
  const todoBackupKey = () => {
    const scope = store.isLogged
      ...
      : "guest";
    return `startdeck-sd-todo-backup-${scope}-${widget.value.id}`;
  };
  ...
  const readLocalBackup = () => {
    ...
    try {
      ...
      if (!raw) return [];
      return normalizeSdTodoWidgetData({ tasks: JSON.parse(raw) }).tasks;
    } catch {
      return [];
    }
  };

  const writeLocalBackup = (items: SdTodoTask[]) => {
    if (!store.isLogged || typeof localStorage === "undefined") return;
    try {
  ...
  const pushSocketUpdate = useDebounceFn((data: SdTodoWidgetData) => {
  ...
  const emitData = (data: SdTodoWidgetData, localMutation = false) => {
    if (localMutation) {
  ...
  const commitTasks = (items: SdTodoTask[], localMutation = true) => {
  ...
  const pollRemote = async (force = false) => {
    ...
    try {
      const raw = await fetchSdTodoWidgetData(widget.value.id, {
        headers: store.getHeaders(),
        signal: controller.signal,
      });
      const nextData = normalizeSdTodoWidgetData(raw);
