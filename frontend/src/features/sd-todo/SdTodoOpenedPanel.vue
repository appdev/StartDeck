...
import { useSdTodoRuntime } from "./useSdTodoRuntime";
import type { SdTodoWidgetData } from "./sdTodoTypes";
...
const emit = defineEmits<{
  updateData: [data: SdTodoWidgetData];
}>();

const widgetRef = computed(() => props.widget);
const runtime = useSdTodoRuntime(widgetRef, (data) =>
...
<template>
  <div ref="panelRef" class="sd-todo-opened-panel is-basic">
...
<style scoped>
.sd-todo-opened-panel {
  display: block;
  width: 998px;
  height: 600px;
  background: var(--sd-theme-todo-todo-opened-panel-surface-01);
  color: var(--sd-theme-todo-todo-opened-panel-text-01);
  overflow: hidden;
}
...
.todo-add {
  ...
  margin: 0 0 12px;
  border-radius: 8px;
  background: var(--sd-theme-todo-todo-opened-panel-surface-02);
}
...
.todo-add > i {
  ...
  color: var(--sd-theme-todo-todo-opened-panel-text-02);
}
...
.todo-scrollbar-thumb {
  ...
  min-height: 42px;
  border-radius: 999px;
  background: var(--sd-theme-todo-todo-opened-panel-surface-03);
  opacity: 0;
  pointer-events: none;
...
.todo-content-frame:hover .todo-scrollbar-thumb {
  background: var(--sd-theme-todo-todo-opened-panel-surface-04);
}
...
.todo-check {
  ...
  height: 16px;
  padding: 0;
  border: 2px solid var(--sd-theme-todo-todo-opened-panel-border-01);
  border-radius: 4px;
  background: transparent;
}
...
.todo-check.done {
  ...
  align-items: center;
  justify-content: center;
  border: 2px solid var(--sd-theme-todo-todo-opened-panel-border-02);
  background: var(--sd-theme-todo-todo-opened-panel-accent-surface-01);
  color: var(--sd-theme-todo-todo-opened-panel-text-03);
}
...
.todo-row-main textarea {
  ...
  outline: 0;
  background: transparent;
  color: var(--sd-theme-todo-todo-opened-panel-text-04);
  font: inherit;
  font-size: 14px;
  line-height: 21px;
  resize: none;
