...
export type SdBingWallpaperResult = {
  entries: SdWallpaperEntry[];
  sourceStatus: string;
  updatedAt?: string;
  count?: number;
...
const cachedResults = new Map<string, SdBingWallpaperResult>();
const pendingRequests = new Map<string, Promise<SdBingWallpaperResult>>();

const normalizeEntry = (entry: SdWallpaperEntry) => ({
  ...entry,
...
export const fetchSdBingWallpapers = async (
  count = 16,
  refresh = false,
  signal?: AbortSignal,
  page = 1,
): Promise<SdBingWallpaperResult> => {
...
export const resetSdBingWallpaperApiCacheForTests = () => {
  cachedResults.clear();
