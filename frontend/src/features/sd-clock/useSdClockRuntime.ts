...
import { normalizeSdClockWidgetData } from "./sdClockModel";
import type { SdClockWidgetData } from "./sdClockTypes";

const formatTwoDigits = (value: number) => String(value).padStart(2, "0");

export const useSdClockRuntime = (
  widgetRef: Ref<WidgetConfig>,
  updateData?: (data: SdClockWidgetData) => void,
) => {
  const now = ref(new Date());
  const showSeconds = ref(
    normalizeSdClockWidgetData(widgetRef.value.data).showSeconds,
  );
  let timer: ReturnType<typeof setInterval> | null = null;

  const data = computed(() =>
    normalizeSdClockWidgetData(widgetRef.value.data),
  );
