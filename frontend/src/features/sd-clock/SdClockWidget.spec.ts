...
import SdClockWidget from "./SdClockWidget.vue";
import { createDefaultSdClockWidget } from "./sdClockModel";
...
describe("clock widget", () => {
  ...
  it.each([
    ...
    (sizeKey, sizeClass, timeCount, showsDate) => {
      const wrapper = mount(SdClockWidget, {
        props: {
          widget: createDefaultSdClockWidget(),
          sizeKey,
        },
      });

      expect(wrapper.attributes("data-sd-clock-size")).toBe(sizeKey);
