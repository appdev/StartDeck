...
const buildValidShell = () => {
  ...
  root.className = "sd-native-widget";
...
describe("WidgetShellContract", () => {
  ...
  it("keeps the default validation profile pinned to replica selectors", () => {
    ...
    expect(validateWidgetShell(root).violations).toContainEqual(
      expect.objectContaining({
        ...
        expected: ".sd-native-widget",
      }),
    );
  });

  it("accepts the StartDeck main shell profile without replica DOM classes", () => {
    const { root } = buildValidStartDeckShell();
    ...
    expect(root.classList.contains("sd-native-widget")).toBe(false);
  });
