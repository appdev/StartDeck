...
import SdAnniversaryOpenedPanel from "./SdAnniversaryOpenedPanel.vue";
import { createDefaultSdAnniversaryWidget } from "./sdAnniversaryModel";
import { anniversaryPreviewSizes } from "./useSdAnniversaryRuntime";

const openedPanelSource = readFileSync(
  "src/features/sd-anniversary/SdAnniversaryOpenedPanel.vue",
  "utf8",
);
const cardSource = readFileSync(
  "src/features/sd-anniversary/SdAnniversaryCard.vue",
  "utf8",
);

describe("anniversary opened panel", () => {
  ...
  it("keeps the source-measured light template list and 2x2 card metrics", () => {
    const wrapper = mount(SdAnniversaryOpenedPanel, {
      props: { widget: createDefaultSdAnniversaryWidget() },
    });

    const templateCards = wrapper.findAll(".anniversary-template-card");
    expect(templateCards).toHaveLength(3);
    const firstThumbnail = templateCards[0]!.find(".sd-anniversary-card");
  ...
  it("emits add data separately from modifying the selected widget", async () => {
    ...
    expect(
      wrapper
        .find(".anniversary-preview-pane [data-sd-anniversary-card-size]")
        .attributes("data-sd-anniversary-card-size"),
    ).toBe("2x4");
    await wrapper
  ...
  it("applies selected image backgrounds to the payday preview immediately", async () => {
    ...
    const previewCard = wrapper.find(
      ".anniversary-preview-pane .sd-anniversary-card",
    );
    ...
    ).toContain("/sd-live-assets/anniversary/yiyan-8.webp");
  });
  ...
  it("opens a source-like custom color picker for text color", async () => {
    ...
    expect(wrapper.find("[data-sd-anniversary-color-picker]").exists()).toBe(
      true,
    );
