...
export const useNetworkStore = defineStore("network", () => {
  ...
  const getHeartbeatInterval = () =>
    configStore.forceNetworkMode === "latency"
      ? NETWORK_HEARTBEAT_INTERVAL_LATENCY
      : NETWORK_HEARTBEAT_INTERVAL;
  const getHeartbeatTimeout = () =>
    configStore.forceNetworkMode === "latency"
      ? NETWORK_HEARTBEAT_TIMEOUT_LATENCY
      : NETWORK_HEARTBEAT_TIMEOUT;
  const getHeartbeatCheckInterval = () =>
    configStore.forceNetworkMode === "latency"
      ? NETWORK_HEARTBEAT_CHECK_INTERVAL_LATENCY
      : NETWORK_HEARTBEAT_CHECK_INTERVAL;
  ...
  const startNetworkHeartbeat = (
    ...
    networkHeartbeatCheckTimer = setInterval(() => {
      const active =
        ...
        Date.now() - lastNetworkHeartbeatAt <= getHeartbeatTimeout();
      ...
      if (res.ok) {
        ...
        if (data.success) {
          if (Array.isArray(data.css))
            configStore.appConfig.customCssList = data.css;
          if (Array.isArray(data.js))
            configStore.appConfig.customJsList = data.js;
  ...
  const wallpaperListPc = useStorage<string[]>("startdeck-wallpaper-list-pc", [
    DEFAULT_WALLPAPER_NAME,
  ]);
  const wallpaperListMobile = useStorage<string[]>(
    "startdeck-wallpaper-list-mobile",
    [DEFAULT_WALLPAPER_NAME],
  );
  ...
  const buildOrderedWallpaperList = (
    list: unknown,
  ...
  const fetchWallpaperLists = async () => {
    const headers = getHeaders();
    const pcEndpoint =
      configStore.appConfig.wallpaperApiPcList || "/api/backgrounds";
    ...
    try {
      ...
      if (pcRes.ok) {
        wallpaperListPc.value = buildOrderedWallpaperList(
          await pcRes.json(),
          configStore.appConfig.pcWallpaperOrder,
        );
      } else {
        wallpaperListPc.value = ensureDefaultWallpaperFirst(
          wallpaperListPc.value,
        );
      }
      if (mobileRes.ok) {
        wallpaperListMobile.value = buildOrderedWallpaperList(
          await mobileRes.json(),
          configStore.appConfig.mobileWallpaperOrder,
        );
      } else {
        wallpaperListMobile.value = ensureDefaultWallpaperFirst(
          wallpaperListMobile.value,
        );
      }
    } catch (error) {
  ...
  const initGlobalDrag = () => {
    ...
    window.addEventListener(
      "dragleave",
      () => {
        if (globalDrag.value.active) {
          globalDrag.value.depth = Math.max(0, globalDrag.value.depth - 1);
          if (globalDrag.value.depth === 0) dragReset();
        }
      },
      true,
    );
    ["drop", "dragend", "pointerup", "mouseup", "blur"].forEach((evt) => {
      window.addEventListener(evt, dragReset, true);
    });
  };
  ...
  const initEventBindings = (
    ...
    if (typeof window !== "undefined") {
      window.addEventListener("beforeunload", () => {
        configStore.isPageUnloading = true;
      });
